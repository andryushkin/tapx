...
- Реализация: `stitchForceColumn()` — сшивает изображения в canvas, вставляет в нормальный поток выше `position:absolute` media-контейнера X.com, показывает как одну `<img>`
