...
TapX — Chrome-расширение (MV3), которое:
...
- Отправляет `dataUrl` (JPEG 0.98) в background service worker → Downloads API.
- Имя файла: `tapx_{username}_{tweetId}_stitched.jpg`.

### FR-3: Toggle вкл/выкл

- Popup с переключателем синхронизирует состояние через `chrome.storage.local`.
...
| Требование | Значение |
|---|---|
| Платформа | Chrome MV3 (Manifest Version 3) |
| Разрешения | `activeTab`, `storage`, `downloads` |
| Хосты | `x.com`, `twitter.com`, `pbs.twimg.com` |
| Приватность | Вся обработка локальная, без трекинга, без серверов |
| Размер | ~19 KB (ZIP для CWS) |
...
- **Только Chrome** — Firefox, Safari не в scope v0.1.0.
