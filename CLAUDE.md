...
3. `processArticle()`:
   ...
   - Определяет layout через `detectLayout(count)` (count-based: 4→2×2, 2→2×1, иначе 1×N)
...
2. ~~**Race condition с lazy load**~~ — **Исправлено:** `detectLayout` переведён на count-based (4→2×2, 2→2×1, иначе 1×N). `getBoundingClientRect()` не используется.
